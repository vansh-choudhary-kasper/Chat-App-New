...
const imageMessageReply = (imageSrc, setReply) => {
  ...
  return (
    <div className="reply_image_main_container">
      ...
      <div className="reply_image_container">
        <img src={imageUrl} alt="reply" className="reply_image_content" />
      </div>
    </div>
  );
};
const videoMessage = (videoSrc, setVideo) => {
  ...
  return (
    <div className="reply_image_main_container reply_pdf">
      ...
      <div className="reply_pdf_container">
        {fileTypes === "pdf" ? <AiOutlineFilePdf /> : <AiOutlineFileZip />}{" "}
        <p>
          {pdfSrc.name.length > 80 ? pdfSrc.name.splice(0, 79) : pdfSrc.name}
        </p>
      </div>
    </div>
  );
...
const Footer = ({ deviceType }) => {
  ...
  const sendHandler = (e) => {
    if (e.key === "Enter" || e.type === "click") {
      ...
      if (msg && msg.trim() && fileTypes) {
        ...
        formData.append("conversation_id", current_conversation);
        formData.append("conversation", "chat");
        formData.append("from", userId);
        formData.append("to", current_user._id);
  ...
  return (
    <div
      className={
        deviceType === "mobile"
          ? "footer_main_container mobileFooter"
          : "footer_main_container"
      }
    >
      {/* {reply ? textMessageReply("rtrrt", setReply) : imageMessageReply(profile,setReply)} */}
      {fileTypes === "video" && videoMessage(fileValue, removeHandler)}
      {fileTypes === "image" && imageMessageReply(fileValue, removeHandler)}
      {(fileTypes === "pdf" || fileTypes === "zip") &&
        pdfMessage(fileValue, fileTypes, removeHandler)}
      <div className="footer_container">
        <div className="footer_icon">
          <IoIosAttach onClick={attachmentHandler} />
          <div
            className={
              showAttach ? "footer_icon_child hidden" : "footer_icon_child"
            }
          >
            <div
              className="footer_select_icon"
        ...
        <div className="footer_input">
          <input
            ...
            disabled={fileTypes === "zip" || fileTypes === "pdf" ? true : false}
            ref={inputRef}
